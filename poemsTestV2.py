...
def printPoem():
    ...
    printer.size = adafruit_thermal_printer.SIZE_MEDIUM # printer.setSize('M')
    printer.print(wrappedTitle) # printer.println(wrappedTitle)
    printer.justify = adafruit_thermal_printer.JUSTIFY_LEFT # printer.justify('L')
    printer.size = adafruit_thermal_printer.SIZE_SMALL # printer.setSize('S')
    printer.print("\n")
    printer.print(wrappedPoem) # printer.println(wrappedPoem)
    printer.print(wrappedAuthor) # printer.println(wrappedAuthor)
    # printer.writeBytes(0x1B, 0x21, 0x1)
    printer.print(randPoem[3]) # printer.println(randPoem[4] +", " + randPoem[0] + ".")
    printer.feed(3)
